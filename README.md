...
An unplanned event occurs when the primary region becomes unhealthy. There is no AWS-orchestrated automated solution available to promote the secondary region and bring the database up and running. This project illustrates a way of achieving this by building the stacks in a certain way along with a series of Lambdas and with the help of a step function  
...
1. A custom KMS Key for the database encryption and for the secret manager encryption. The ARN of the the KMS Key will be exported a parameter store entry called /demo/rds/kmskey/arn
2. Two secret manager entries. One as the root user ('theadmin') and another user called 'theuser' for the applications. These secretes will be exported the parameter store (demo/rds/theadmin/secret and demo/rds/theuser/secret respectively)
...
3. A parameter store entry called '/demo/rds/global/cluster/name' to store the GLoabl Cluster Name. We will be changing this value when we do a fallback after an unplanned failover
...
1.  aws cloudformation update-stack --stack-name aurora-pg-rds-iam --template-body file://stack-iam.yml --profile saml --region us-east-1 --capabilities CAPABILITY_AUTO_EXPAND CAPABILITY_NAMED_IAM
2.  aws cloudformation update-stack --stack-name aurora-pg-rds-iam --template-body file://stack-iam.yml --profile saml --region us-west-2 --capabilities CAPABILITY_AUTO_EXPAND CAPABILITY_NAMED_IAM
...
1.  aws cloudformation create-stack --stack-name aurora-pg-rds-database --template-body file://stack-db-east.yml --profile saml --region us-east-1 --capabilities CAPABILITY_AUTO_EXPAND CAPABILITY_NAMED_IAM
